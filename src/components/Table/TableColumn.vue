<template>
  ...
  >
    <template slot-scope="scope">
      <template v-if="editor === 'indexMove'">
        <el-button v-if="scope.$index === scope._self.mouseOnIndex" @click="moveUp(scope)" type="text">上移</el-button>
        <span>{{ scope.$index + 1 }}</span>
        <el-button v-if="scope.$index === scope._self.mouseOnIndex" @click="moveDown(scope.$index)" type="text">下移</el-button>
      </template>
      ...
      <template v-else-if="editor === 'timeText'">
        <div class="tableColumnInputStyle02">
          <span>{{scope.row[name] | YYYYmmDD_Filter}}</span>
        </div>
      </template>
      ...
      <template v-else-if="editor === 'date'">
        <div class="after tableColumnInputStyle">
          <el-date-picker
            ...
            :type="dateType"
            :clearable="false"
            :disabled="isdisabled || (toDisabled && scope.row.flag == '2')"
            v-model="scope.row[name]"
...
export default {
  ...
  created() {
    this.selectoptions = Dictionaries[this.name] // 查询对应加载项 selectoptions
  ...
  watch: {
    ...
    //     this.tableData.indexOf(this.row) !== '-1' &&
    //     this.tableData.indexOf(this.row) === value
    //   ) {
    //     let obj = {
  ...
  methods: {
    disabledFn(data, scope) {
      return this.$emit('disabledFn', data, this.name, scope)
    },
    date() {
    ...
    moveUp(scope) {
      ...
      if (index >= 0) {
        ...
        if (newIndex >= 0) {
          let oldData = this.tableData[index]
          let newData = this.tableData[newIndex]
          this.tableData[index] = newData
          this.tableData[newIndex] = oldData
          this.tableData.push()
        }
      }
    },
    ...
    changeValue(row, key, value) {
      this.$nextTick(() => {
        ...
        } else if (this.inputtype === 'number2') { // number2 输入.01 返回0.01
        ...
        } else if (this.inputtype === 'number3') { // number3 数字允许三位小数
          if (/^(0|([1-9][0-9]*)|(([0]\.\d{0,3|[0-9][0-9]*\.\d{0,3})))$/.test(value.target.value.trim())) {
            if (value.target.value.trim().indexOf('.') > 0 && value.target.value.trim().length > 16) {
              row[key] = value.target.value.trim().split('.')[0].slice(0, 16) + '.' + value.target.value.trim().split('.')[1]
            } else {
              row[key] = value.target.value.trim().slice(0, this.maxlength)
        ...
        } else if (this.inputtype === 'zerotoone2') { // 0.00 - 1.00 输入.1显示0.1
          if (value.target.value && value.target.value[0] === '.') {
            value.target.value = '0.'
          }
          if (/^(0|1|(1\.0{0,2})|(0\.\d{0,2}))$/.test(value.target.value)) {
        ...
        } else if (this.inputtype === 'zerotoone1') { // 0.000 - 1.000
          if (/^(0|1|(1\.0{0,3})|(0\.\d{0,3}))$/.test(value.target.value)) {
            row[key] = value.target.value.trim().slice(0, this.maxlength)
          } else {
            row[key] = value.target.value.substring(0, value.target.value.length - 1)
          }
