<template>
  <el-col :span="Number(colspan)">
    ...
    >
      ...
      >
        <el-button v-if="rightBtn" @click="rightBtnClick" slot="append" :icon="btnIcon" :style="{backgroundColor:isBtnColor?'#fff':''}"></el-button>
  ...
  methods: {
    changeValue(val) {
      ...
      this.$nextTick(() => {
        ...
        } else if (this.inputtype === 'number1') {
          // 小数后两位,可以为负数
          if (val === '-' || /^-?(([1-9][0-9]*)|(([0]\.\d{0,2}|[1-9][0-9]*\.\d{0,2})))$/.test(val.trim())) {
            this.inputvalue = val.trim().slice(0, this.maxlength)
          } else {
            this.inputvalue = val.substring(0, val.length - 1)
          }
        ...
        } else if (this.inputtype === 'zerotoone2') {
          // 0.0000 - 1.0000
          if (/^(0|1|(1\.0{0,4})|(0\.\d{0,4}))$/.test(val)) {
            this.inputvalue = val.trim().slice(0, this.maxlength)
          } else {
            this.inputvalue = val.substring(0, val.length - 1)
          }
    ...
    onChange(data) {
      ...
      } else if (this.fuMoney1) {
        if (data) {
          this.inputvalue = this.dealMoney(data)
        }
      } else if (this.wanMoney) {
      }
      this.$emit(
